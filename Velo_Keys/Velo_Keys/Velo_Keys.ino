...
void TurnLeftCheck()
{
  ...
  else if (LeftState == TURN_OFF)
  {
    ...
    if (!digitalRead(STOP_PIN))
    {
      for (uint8_t j = TURN_LEFT_PIXEL_START; j < (TURN_LEFT_PIXEL_START + TURN_LEFT_PIXEL_NUM); j++)
        LEDS.setPixelColor(j, STOP_ACTIVE_COLOR);
    }
...
void TurnRightCheck()
{
  ...
  else if (RightState == TURN_OFF)
  {
    ...
    if (!digitalRead(STOP_PIN))
    {
      for (uint8_t j = TURN_RIGHT_PIXEL_START; j < (TURN_RIGHT_PIXEL_START + TURN_RIGHT_PIXEL_NUM); j++)
        LEDS.setPixelColor(j, STOP_ACTIVE_COLOR);
    }
